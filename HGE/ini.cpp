...
void CALL HGE_Impl::Ini_SetInt(const char *section, const char *name, int value)
{
	...
	{
		sprintf(buf,"%d",value);
		WritePrivateProfileString((LPTSTR)section, (LPTSTR)name, (LPTSTR)buf, (LPTSTR)szIniFile);
	}
}
...
int CALL HGE_Impl::Ini_GetInt(const char *section, const char *name, int def_val)
{
	...
	if(szIniFile[0]) {
		if(GetPrivateProfileString((LPTSTR)section, (LPTSTR)name, TEXT(""), (LPTSTR)buf, sizeof(buf), (LPTSTR)szIniFile))
		{ return atoi(buf); }
		else { return def_val; }
	}
	return def_val;
...
{
	if(szIniFile[0]) WritePrivateProfileString((LPTSTR)section, (LPTSTR)name, (LPTSTR)value, (LPTSTR)szIniFile);
}


char* CALL HGE_Impl::Ini_GetString(const char *section, const char *name, const char *def_val)
{
	if(szIniFile[0]) GetPrivateProfileString((LPTSTR)section, (LPTSTR)name, (LPTSTR)def_val, (LPTSTR)szIniString, sizeof(szIniString), (LPTSTR)szIniFile);
