...
bool CALL HGE_Impl::System_Initiate()
{
	...
	winclass.lpszClassName	= (LPTSTR)WINDOW_CLASS_NAME;
	if(szIcon) winclass.hIcon = LoadIcon(hInstance, (LPTSTR)szIcon);
	...
	if(bWindowed)
		hwnd = CreateWindowEx(0, (LPTSTR)WINDOW_CLASS_NAME, (LPTSTR)szWinTitle, styleW,
				rectW.left, rectW.top, rectW.right-rectW.left, rectW.bottom-rectW.top,
				hwndParent, NULL, hInstance, NULL);
	else
		hwnd = CreateWindowEx(WS_EX_TOPMOST, (LPTSTR)WINDOW_CLASS_NAME, (LPTSTR)szWinTitle, styleFS,
				0, 0, 0, 0,
...
//	_SoundDone();
	...
	if(hInstance) UnregisterClass((LPTSTR)WINDOW_CLASS_NAME, hInstance);
...
void CALL HGE_Impl::System_SetStateInt(hgeIntState state, int value)
{
	switch(state)
	{
		case HGE_SCREENADAPTERID: if(!pD3DDevice) nScreenAdapterID=value; break;
...
void CALL HGE_Impl::System_SetStateString(hgeStringState state, const char *value)
{
	...
	switch(state)
	{
		case HGE_ICON:			szIcon=value;
								if(pHGE->hwnd) SetClassLong(pHGE->hwnd, GCL_HICON, (LONG)LoadIcon(pHGE->hInstance, (LPTSTR)szIcon));
								break;
		case HGE_TITLE:			strcpy(szWinTitle,value);
								if(pHGE->hwnd) SetWindowText(pHGE->hwnd, (LPTSTR)szWinTitle);
								break;
...
int CALL HGE_Impl::System_GetStateInt(hgeIntState state)
{
	switch(state)
	{
		case HGE_SCREENADAPTERID: return nScreenAdapterID;
...
{
	if((DWORD)ShellExecute(pHGE->hwnd, NULL, (LPTSTR)url, NULL, NULL, SW_SHOWMAXIMIZED)>32) return true;
	else return false;
}
...
void CALL HGE_Impl::System_Snapshot(const char *filename)
{
	...
	if(pD3DDevice)
	{
		...
		D3DXSaveSurfaceToFile((LPTSTR)filename, D3DXIFF_BMP, pSurf, NULL, NULL);
		pSurf->Release();
	}
}
...
// 	streams=0;
	...
	nScreenAdapterID=D3DADAPTER_DEFAULT;
	nScreenWidth=800;
	nScreenHeight=600;
	nScreenBPP=32;
	bWindowed=false;
	...
	GetModuleFileName(GetModuleHandle(NULL), (LPTSTR)szAppPath, sizeof(szAppPath));
	int i;
