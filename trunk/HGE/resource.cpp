...
_fromfile:
	...
	hF = CreateFile((LPTSTR)Resource_MakePath(filename), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_RANDOM_ACCESS, NULL);
...
char* CALL HGE_Impl::Resource_EnumFiles(const char *wildcard)
{
	if(wildcard)
	{
		...
		hSearch=FindFirstFile((LPTSTR)Resource_MakePath(wildcard), &SearchData);
...
char* CALL HGE_Impl::Resource_EnumFolders(const char *wildcard)
{
	if(wildcard)
	{
		...
		if((SearchData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
		   strcmp((char*)SearchData.cFileName,".") && strcmp((char*)SearchData.cFileName,".."))
