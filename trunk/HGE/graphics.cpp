...
bool CALL HGE_Impl::Gfx_BeginScene(HTARGET targ)
{
	...
	else if (hr == D3DERR_DEVICENOTRESET)
	{
		if(bWindowed)
		{
			if(FAILED(pD3D->GetAdapterDisplayMode(nScreenAdapterID, &Mode)) || Mode.Format==D3DFMT_UNKNOWN) 
			{
...
void HGE_Impl::_SetBlendMode(int blend)
{
	...
	if((blend & BLEND_ALPHABLEND_DISABLE) != (CurBlendMode & BLEND_ALPHABLEND_DISABLE))
	{
		if(blend & BLEND_ALPHABLEND_DISABLE)
			pD3DDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE);
		else
			pD3DDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);
	}			

	if((blend & BLEND_ALPHATEST_DISABLE) != (CurBlendMode & BLEND_ALPHATEST_DISABLE))
	{
		if(blend & BLEND_ALPHATEST_DISABLE)
...
// Get adapter info
	...
	pD3D->GetAdapterIdentifier(nScreenAdapterID, 0, &AdID);
...
// Create D3D Device
	...
	if( FAILED( pD3D->CreateDevice( nScreenAdapterID, D3DDEVTYPE_HAL, hwnd,
